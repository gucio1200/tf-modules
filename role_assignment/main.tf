  ...
  flat_assignments = flatten([
    for k, v in var.assignments : [
      for p_idx, pid in(
        ...
        for r_idx, role in(
          v.role_definition_name != null ? try(tolist(v.role_definition_name), [v.role_definition_name]) :
          v.role_definition_id != null ? try(tolist(v.role_definition_id), [v.role_definition_id]) : []
          ) : [
